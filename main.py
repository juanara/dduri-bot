...
# [자체 엔진] 외부 패키지 의존성 없는 무적 스케줄러 (무삭제 보존)
async def custom_scheduler_loop(application):
    await asyncio.sleep(5)
    bot = application.bot
    logger.info("🚀 뜌리봇 무적 스케줄 엔진 가동!")
    while True:
        try:
...
def get_menu_recommendation(command):
    ...
    return f"🍴 <b>추천 메뉴</b>: <b>{random.choice(starbucks_30 if '/커추' in command else lunch_30)}</b>"
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not update.message.from_user.is_bot:
        ...
        if any(w in text_lower for w in bad_words) or any(w in clean_text for w in bad_words):
            ...
            if os.path.exists("2.webm"):
                try: s_msg = await context.bot.send_sticker(chat_id, open("2.webm", "rb")); s_id = s_msg.message_id
                except: pass
    ...
    if text in ["/주사위", "!주사위"]:
        ...
        return
    if text_lower.startswith(("/all", "/전체공지", "/전체멘션")) and (await is_authorized(update, context)):
    ...
    if uid == ADMIN_ID and is_private:
        ...
        if text_lower == "/카운트확인":
            summary = [f"🏠 <b>{r.get('room_name','?')}</b>: {r.get('msg_count',0):,}회" for r in list(col_members.find())]
    ...
    if text.startswith(('/', '!')):
        ...
        room = col_members.find_one({"chat_id": str(chat_id)}); target = (room.get("local_commands", {}).get(cmd) if room else None) or load_bot_data().get("commands", {}).get(cmd)
...
async def save_logic_with_delay(chat_id, context, m_id, message=None):
    ...
    if not t_id: return await context.bot.send_message(chat_id, "⚠️ 방을 선택하세요.")
    try:
        if "/스케줄등록" in raw_html:
            ...
            data = {"chat_id": t_id, "name": h[0], "start_dt": h[1], "end_dt": h[2], "slot_start": h[3].replace("-","").strip()[:4], "slot_end": h[3].replace("-","").strip()[-4:], "interval": int(intv), "data": {"photos": photos, "caption": content.strip()}}
            ...
            cmd_data = {"photos": (media_group_cache[m_id]["ids"] if m_id else []), "caption": msg.strip(), "buttons": re.sub('<[^<]+?>', '', btn).strip()}
            if t_id == "common": cmds = load_bot_data().get("commands", {}); cmds[key] = cmd_data; save_bot_data(cmds)
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("set_room:"):
        ...
        await query.edit_message_text(f"🎯 설정 활성화 (ID: {r_id})", reply_markup=InlineKeyboardMarkup(btns))
    elif data.startswith("show_list:"):
        r_id = data.split(":")[1]; target = load_bot_data().get("commands", {}) if r_id == "common" else (col_members.find_one({"chat_id": r_id}).get("local_commands", {}) if col_members.find_one({"chat_id": r_id}) else {})
        ...
        if r_id == "common": cmds = load_bot_data().get("commands", {}); cmds.pop(k, None); save_bot_data(cmds)
        ...
        btns = [[InlineKeyboardButton("📁 [공용] 설정", callback_data="set_room:common")]] + [[InlineKeyboardButton(f"🏠 {r['room_name']}", callback_data=f"set_room:{r['chat_id']}") ] for r in list(col_members.find()) if "room_name" in r]
